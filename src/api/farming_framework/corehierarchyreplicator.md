...
| ------------------- | ----------- | ----------- | ---- |
| `ActivateReplicator(string, boolean|nil)` | `None` | On the server, this activates the replicator for all players who want to receive from it. On the client, it will activate the replicator as long as the replicator is active on the server. | None |
...
| `ClearCurrentInstance()` | `None` | Called after an instance is spawned to ensure you can't accidentally get a nil instance. | None |
