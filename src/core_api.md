...
| `Game.playerLeftEvent` | Event&lt;Player&gt; | Fired when a player has disconnected from the game or their character has been destroyed. If `Game.GetPlayers()` is used in the event callback, the player that left will still be included in the array. [:fontawesome-solid-info-circle:](../api/examples/#gameplayerleftevent "Game: playerLeftEvent Example") | None |
