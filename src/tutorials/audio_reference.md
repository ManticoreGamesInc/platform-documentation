...
Note: As a reminder for smart properties, they can be adjusted and modified via script using: `GetSmartProperty(string)` or `SetSmartProperty(string, value)`. See API documentation regarding Smart properties [here](../api/index.md#smartobject). When referencing drop-down strings, keep in mind that Lua is case-sensitive and they must be referenced exactly as they are listed.
